...
int main(int argn, char **argc)
{
    ...
    fps = ((cap.get(CV_CAP_PROP_FPS) < 33.0 && cap.get(CV_CAP_PROP_FPS) > 1.0) ? cap.get(CV_CAP_PROP_FPS) : 33.0);
    ...
    while(key != 'q') 
    {
        ...
        if (size_frame.width > 0 && size_frame.height > 0 && resize)
        ...
        else 
        {
            ...
            if (find_circles)
            {
                ...
                cv::equalizeHist(img_eye_bb, img_eye_bb);
                //cv::medianBlur(img_eye_bb, img_eye_bb, 3);
                cv::Canny(img_eye_bb, img_eye_bb, 200, 230);
                //cv::threshold(img_eye_bb, img_eye_bb,20,255,cv::THRESH_BINARY_INV);
                //cv::erode(img_eye_bb, img_eye_bb, cv::Mat());
                cv::imshow("teste",img_eye_bb);
                /// Apply the Hough Transform to find the circles
                cv::HoughCircles( img_eye_bb, circles, CV_HOUGH_GRADIENT, 1, 5.0, 30, 150, 1, 25 );
        ...
        cv::putText(frame,std::to_string(contaPiscada) + std::string(" piscadas | circulos: ") + std::to_string(circles.size()) ,
